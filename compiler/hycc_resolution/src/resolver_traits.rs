...
pub trait ResolveIdentArgs<TEx, E>: ResolveTy<E> + ResolveExpr<TEx, E> {
    fn resolve_ident_args(&mut self, arguments: &HirIdentArguments) -> Result<Vec<GenericArg>, E> {
        let mut args = Vec::new();
        for argument in &arguments.data {
            match &argument {
                HirIdentArgument::Ty(ty) => args.push(GenericArg::Ty(self.resolve_ty(&ty)?)),
                HirIdentArgument::Expr(expr) => todo!("resolve expr ident args"),
            }
        }
